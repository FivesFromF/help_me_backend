...
# =============================================
# Post Authentication Lambda: Audit sign-in events
# =============================================

resource "aws_iam_role" "post_authentication_role" {
  ...
  tags = {
    Project   = "HelpMe"
    Component = "Lambda-PostAuthentication"
  }
}

resource "aws_iam_role_policy_attachment" "post_authentication_basic" {
  role       = aws_iam_role.post_authentication_role.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_iam_policy" "post_authentication_eventbridge" {
  name = "${var.project_name}-post-authentication-eb-policy"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect   = "Allow"
...
resource "aws_iam_role_policy_attachment" "post_authentication_eb_attach" {
  role       = aws_iam_role.post_authentication_role.name
  policy_arn = aws_iam_policy.post_authentication_eventbridge.arn
}

resource "aws_lambda_function" "post_authentication" {
  filename      = "${path.module}/post_authentication.zip"
  function_name = "${var.project_name}-post-authentication"
  role          = aws_iam_role.post_authentication_role.arn
  handler       = "index.main"
  runtime       = "nodejs20.x"

  environment {
    variables = {
      CORE_SYSTEM_BUS_NAME = var.core_system_bus_name
    }
  }
