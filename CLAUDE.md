...
- **`.env` also shadows your real AWS credentials.** It sets `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` to `test` for the local emulators, and env vars outrank `~/.aws/credentials` — so any inheriting shell fails every real call (`aws`, `terraform`, `docker push`) with `InvalidClientTokenId`. Prefix with `env -u AWS_ACCESS_KEY_ID -u AWS_SECRET_ACCESS_KEY`, and confirm with `aws sts get-caller-identity` before blaming your credentials. Note that `!` in the Claude Code prompt runs **bash**, so `$env:VAR=$null` there is a silent no-op.
...
- **Terraform consumes Lambda zips that git does not track.** `node build.js` writes them into `infra/modules/lambda/`, and `.gitignore` excludes `*.zip` — so a fresh clone has **none of them** and `terraform apply` fails on a missing `filename` until `npm run build` has run. On a working tree the opposite bites: a stale local zip applies silently as old code. Build first, always.
- **Not everything in `infra/` is live.** `infra/main.tf` instantiates 13 modules and `infra/modules/` holds exactly those 13 — a zip or a directory is not evidence of a deployment, `terraform state list` (103 resources) is. Cleared out on 2026-08-23: the `apigateway` and `authorizer` modules (never instantiated), the `authorizer` `build.js` entry (no handler source, skipped silently by an `fs.existsSync` guard), and seven unreferenced zips from the same API-Gateway-era stack — an older working tree may still hold copies. [[Architecture/Code_Layout]]
- **Infra changes have their own trap list.** Read the Traps section of [[Runbooks/Cloud_Deployment]] before any `terraform apply` — the RDS instance carries `prevent_destroy` and is keyed in state by `DbiResourceId` (not identifier, so a restore needs `state rm` + `import`), and every RDS security-group rule must stay a standalone `aws_security_group_rule` because one inline `ingress` block is treated as the complete set and silently deletes the AI worker's access to Postgres.
- **Windows shells mangle AWS CLI arguments.** In Git Bash, MSYS path conversion rewrites leading-slash values — `/ecs/helpme-ai` becomes a Windows path and fails the log-group name constraint, and CIDRs inside security-group-rule import ids break the same way; prefix with `MSYS_NO_PATHCONV=1`. In PowerShell, `-target=aws_security_group_rule.x` is split unless the whole token is quoted, and Terraform then reports `Invalid target "aws_security_group_rule"`. [[Runbooks/Cloud_Deployment]]
